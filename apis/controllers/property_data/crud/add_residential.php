...
//check
    ...
    if($result=="Success")
    {
        ...
        {
            case $error_occured:
                $response->send_response(503,"Error Adding Office -> "/*.$review*/,NULL);
                $app_con ->rollback();
                return;
            break;
            ...
            default:
                ...
                if (!($error_occured==true)) 
                {
                    ...
                    $logs->STATEMENT= "Added property : Property Type -> Residence, Region->".$property_data->REGION.", District->".$property_data->DISTRICT.", Town->".$property_data->TOWN.", Allotee->".$property_data->ALLOTEE;
                    $logs->DATE= $ladate;
                    $logs->TIME= $latimez;
                    ...
                    if (!($add_log=="Success")) 
                    {
                        $error_occured=true;
                        $app_con ->rollback();
                        $response->send_response(503,"Error Adding Residence -> "/*.$add_log*/,NULL);
                        return;
                    }
                    else
                    {   
                        $app_con -> commit();
                        $response->send_response(201,"Residence Added Successfully",NULL);
                        return;
                    }
