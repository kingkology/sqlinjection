...
$residence->DISTRICT=$_SESSION['user_district'];

if (strtolower($filter_field)=="district") 
{
	$filter_field=$residence->DISTRICT;
}

if (strtolower($filter_field)=="region") 
{
	$filter_field=$residence->REGION;
}


$residence->QUERY_FIELD=str_replace(" ", "_", strtolower($filter_field));
$residence->QUERY_TYPE=$the_type;
$residence->QUERY_VALUE=$filter_value;


$result=$residence->search_property_region($app_con);
...
<?php
	...
	while($row = $result->fetch_assoc()) 
	{
		...
		<td style="width:100px" class='col_res'><?php echo ($res_type=="Other") ? $row['resident_type_other'] : $res_type ; ?></td>
