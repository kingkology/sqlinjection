<?php
  ...
  // required headers for post api
    ...
    {
        $pin = mysqli_real_escape_string($sys_con,$_GET['pin']);
    }
    else
    {

      $response->send_response(400,"Unable to search record. Staff NID is required",NULL);
      return;
    }
    ...
    $result=$staff->get_staff_by_id($hr_con);
    ...
    if ($result) 
    {
      if($result->num_rows>0)
      {
        ...
        $data_item=array("th1" =>"No.", "th3" =>"Name","th4" =>"Contact","th5" =>"Office","th6" =>"Department","th7" =>"Status","th8" =>"Action");
        $new_data[]=$data_item;
        ...
        while($row = $result->fetch_assoc()) 
        {
            ...
            $staff_date_joined=$row['staff_date_joined'];
    ...
    {
      $response->send_response(404,"No data Found".$result,NULL);
      return; 
    }
